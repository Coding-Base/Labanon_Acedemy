...
export default function SubjectSelectionModal({
  ...
  const fetchSubjects = async () => {
    ...
    try {
      ...
      setSelectedSubjects([]) // Reset selection when modal opens
    } catch (err: any) {
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      // Check activation for all selected subjects
      for (const subject of selectedSubjects) {
        ...
        if (!res.ok || !data.unlocked) {
          // Redirect to activation if any subject is locked
      ...
      // All subjects are unlocked, proceed to next step
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] flex flex-col shadow-xl">
        ...
        <div className="p-6 border-t border-gray-100 flex-shrink-0 bg-gray-50 rounded-b-lg">
          ...
          <div className="flex gap-3">
            ...
            >
              {checkingActivation ? 'Verifying Access...' : 'Continue'}
            </button>
          </div>
        </div>
