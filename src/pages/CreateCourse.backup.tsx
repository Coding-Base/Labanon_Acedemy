...
export default function CreateCourse() {
  ...
  const publishCourse = () => {
    // Check verification status for tutor and institution accounts
    if (verificationStatus && !verificationStatus.is_verified && verificationStatus.verification_status !== 'not_applicable') {
      alert(`You cannot publish courses until your account is verified by the master admin.\n\nReason: ${verificationStatus.reason || 'Verification pending'}`)
      return
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-3">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-2 md:mt-0">
              ...
              <button 
                ...
                disabled={publishing || (!verificationLoading && verificationStatus && !verificationStatus.is_verified && verificationStatus.verification_status !== 'not_applicable')}
                className={`px-5 py-2 rounded-lg font-semibold text-sm transition ${
                  (!verificationLoading && verificationStatus && !verificationStatus.is_verified && verificationStatus.verification_status !== 'not_applicable')
                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                    : 'bg-brand-600 text-white hover:bg-brand-700'
                } disabled:opacity-50`}
                title={(!verificationLoading && verificationStatus && !verificationStatus.is_verified && verificationStatus.verification_status !== 'not_applicable') ? verificationStatus.reason : 'Publish this course'}
              >
